...
EYE_AR_CONSEC_FRAMES = 10  # 连续满足EAR阀值的帧数阀值
EYE_CLOSE_THRESH = 60  # 闭眼总计数阀值,超出后发出告警
MAR_THRESH = 0.6  # 打哈欠长宽比阀值
MOUTH_AR_CONSEC_FRAMES = 60  # 连续满足MAR阀值的帧数阀值
# 初始化闭眼帧计数器和闭眼总数
eCOUNTER = 0
eTOTAL = 0
# 初始化打哈欠帧计数器和打哈欠总数
...
while True:  # 从视频流循环视频帧
    ...
    rects = detector(gray, 0) #获取脸部位置
    
    for rect in rects:  # 循环脸部位置信息
        landmarks = predictor(gray, rect)  # 获取脸部特征位置的信息
        shape = face_utils.shape_to_np(landmarks)  #转换为数组的格式
        ...
        '''
            判断是否打瞌睡,如果EAR小于阈值,则加1,如果连续EYE_AR_CONSEC_FRAMES次都小于阈值,则表示进行了一次闭眼活动,闭眼数到达阈值后发出告警
        '''
        if ear < EAR_THRESH:  # 循环,满足条件的,闭眼帧计数器+1
            eCOUNTER += 1
            if eCOUNTER >= EYE_AR_CONSEC_FRAMES:  # 连续数帧都闭眼则判断为一次眨眼活动
                eTOTAL += 1
                if eTOTAL >= EYE_CLOSE_THRESH :        
                    cv2.putText(frame, "Drowsiness!!!", (80, 200),cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)  # 在屏幕上显示Drowsiness提醒
                    if not ALARM_ON:
                        ALARM_ON = True  # 开启告警
                        if args["alarm"] !="":
        ...
        cv2.putText(frame, "Blinks: {}".format(eTOTAL), (150, 30),cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)   # 在屏幕上显示闭眼总数
        cv2.putText(frame, "eCOUNTER: {}".format(eCOUNTER), (300, 30),cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)    # 在屏幕上显示闭眼帧计数器
        cv2.putText(frame, "EAR: {:.2f}".format(ear), (450, 30),cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)   # 在屏幕上实时显示眼睛长宽比   
        '''
            判断是否打哈欠,如果MAR小于阈值,则加1,如果连续MOUTH_AR_CONSEC_FRAMES次都小于阈值,则表示打了一次张嘴，张嘴数到达阈值后告警
        '''
        ...
        right = rect.right()
        bottom = rect.bottom()
        cv2.rectangle(frame, (left, top), (right, bottom), (0, 255, 0), 3)  # 用矩形框标注人脸
...
#    print('嘴巴实时长宽比:{:.2f} '.format(mar)+"\t是否张嘴:"+str([False,True][mTOTAL>=60]))
    ...
    cv2.putText(frame, "Press 'q' to Quit", (10, 370),cv2.FONT_HERSHEY_SIMPLEX, 0.7, (84, 255, 159), 2)      # 按q退出
